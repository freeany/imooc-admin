...
7. 添加 `commitlint` 的 `hook` 到 `husky`中，并指令在 `commit-msg` 的 `hooks` 下执行 `npx --no-install commitlint --edit "$1"` 指令
...
**通过husky来监听git hooks钩子，在钩子比如commit-msg钩子里面来执行commitlint来执行对提交代码规范的检测。**
